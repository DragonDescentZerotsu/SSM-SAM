...
class SAM(nn.Module):
    def __init__(self, inp_size=None, encoder_mode=None, loss=None):
        ...
        self.image_encoder = ImageEncoderViT(  # in image_encoder.py
            img_size=inp_size,
    ...
    def forward(self):
        ...
        img_resize = F.interpolate(self.input[:, 0].unsqueeze(1).to(self.device),
                                   scale_factor=self.features.shape[2] / self.input.shape[2],
                                   mode='bilinear')
