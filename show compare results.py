...
with open(file_path, "r") as fb:
    ...
    for line in fb:
        ...
        if (line == "meta trained on 5 imgs, tested on liver, best_dice.pth:"
            ...
            flag = 3
            count = 30
        elif line == 'OSAM, trained on 65 imgs, 100 epochs, tested on liver, best dice:' and count <= 0:
            flag = 4
            count = 30
        if count > 0:
            if (flag == 1 and line != "meta trained on 5 imgs, tested on liver, best_dice.pth:"
            ):
                list1.append(float(line.split(",")[2].split(":")[-1]))
                count -= 1
            elif flag == 2 and line != "original, tested on liver (5 imgs) to get dice:":
                list2.append(float(line.split(",")[2].split(":")[-1]))
                count -= 1
                ...
                count -= 1
            elif flag == 4 and line != "OSAM, trained on 65 imgs, 100 epochs, tested on liver, best dice:":
    ...
    sns.lineplot(x="x", y="list1", data=df, label="Meta SAM best Dice", color='#0000ff', errorbar='sd', linestyle='--',
                 marker='s')
    sns.lineplot(x="x", y="list2", data=df, label="Adapter-SAM no pre-training", color='#18a400', errorbar='sd', linestyle='-',
                 marker='o')
    sns.lineplot(x="x", y="list3", data=df, label="Adapter-SAM with pre-training best loss", color='#ff0000', errorbar='sd', linestyle='-.',
                 marker='v')
