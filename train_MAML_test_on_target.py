...
def make_data_loaders_dict(all_task_list):
    ...
    for task in all_task_list:
        ...
        train_loader, eval_loader = make_data_loaders()
        train_loader_dict[task] = train_loader
        eval_loader_dict[task] = eval_loader
        print(config['train_dataset']['dataset']['args']['root_path_1'])  # 为了调试
    return train_loader_dict, eval_loader_dict
...
def eval_psnr(loader, model, eval_type=None):
    ...
    pbar = tqdm(total=min(len(loader) * config['test_dataset']['batch_size'], config['out_eval_INR']), leave=True,
                desc='evaluation')
    ...
    with torch.no_grad():
        for i, batch in enumerate(loader):
            if (batch['inp'].shape[0] != config['train_dataset']['batch_size'] or (i + 1) * config['train_dataset'][
            ...
            pred = torch.sigmoid(model(inp, batch_gt, num_points=5))
...
def train(all_task_list, train_loader_dict, eval_loader_dict, model, loss_mode):
    model.train()
    ...
    while task_batch_index < len(all_task_list):
        if task_batch_index + task_batch_size < len(all_task_list):  # TODO:检查这里的逻辑
        ...
        for i, task in enumerate(train_task_list):
            ...
            train_loader, eval_loader = train_loader_dict[task], eval_loader_dict[task]
            pbar = tqdm(total=min(config['train_img_number_restrict'] * inner_epochs,
                                  config['train_dataset']['batch_size'] * len(train_loader) * inner_epochs), leave=True,
            ...
            for inner_epoch in range(inner_epochs):
                for j, batch in enumerate(train_loader):
                    ...
                    batch_loss.backward()
                    model_inner.optimizer.step()  # TODO: 学习率得设置成fixed，应该要给optimizer改了
            ...
            ebar = tqdm(
                total=min(config['eval_img_number_restrict'], config['val_dataset']['batch_size'] * len(eval_loader)),
                leave=True,
            ...
            for k, batch in enumerate(eval_loader):
                ...
                gt = batch['gt'].to(device)  # ground truth
                # print('到这了')
                pred = model_inner(inp, gt, num_points=5)
                if loss_mode == 'bce':
                    criterionBCE = torch.nn.BCEWithLogitsLoss()
                elif loss_mode == 'bbce':
                    criterionBCE = BBCEWithLogitLoss()
...
def main(config_, save_path, args):
    ...
    exclude_task_list = ['USF_shadow', 'SBU_shadow', 'ISTD_shadow', 'polyp', 'COD10K', 'cup', 'BraTS20', 'all_pre_train_data',
                         'ISBI', 'transparent', 'all_train_meta_data', '__MACOSX', 'large_test', '.DS_Store', eval_task]
    all_train_task_list = list(set(all_task_list) - set(exclude_task_list))
    train_loader_dict, eval_loader_dict = make_data_loaders_dict(all_train_task_list)

    config['test_dataset']['dataset']['args']['root_path_1'] = os.path.join(config['data_path'], eval_task, 'eval',
                                                                            'img')
    config['test_dataset']['dataset']['args']['root_path_2'] = os.path.join(config['data_path'], eval_task, 'eval',
                                                                            'mask')
    val_loader = make_data_loader(config.get('test_dataset'), tag='test')  # 给下面eval_psnr用
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        train_loss_G = train(all_train_task_list, train_loader_dict, eval_loader_dict, model,
                             loss_mode)
        lr_scheduler.step()
        ...
        if epoch_val is not None:
            if epoch % epoch_val == 0:
                ...
                # result3_list = []
                # result4_list = []
                # for task_eval in all_train_task_list:
                # val_loader = eval_loader_dict[task_eval]
                result1, result2, result3, result4, metric1, metric2, metric3, metric4 = eval_psnr(val_loader,
                                                                                                   model,
                ...
                # result1 = np.mean(result1_list)
                # result2 = np.mean(result2_list)
                # result3 = np.mean(result3_list)
                # result4 = np.mean(result4_list)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default="/root/SAM_Adapter_MAML/configs/train_5_eval_on_target.yaml")
