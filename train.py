...
def eval_psnr(loader, model, eval_type=None):
    ...
    pbar = tqdm(total=len(loader) * config['test_dataset']['batch_size'], leave=True, desc='evaluation')

    pred_list = []
    gt_list = []
    with torch.no_grad():
        for i, batch in enumerate(loader):
            if batch['inp'].shape[0] != config['train_dataset']['batch_size']:
                break
            ...
            pred = torch.sigmoid(model(inp, batch_gt, num_points=20))
...
def train(train_loader, model, loss_mode):
    ...
    model.train()
    pbar = tqdm(
        total=len(train_loader) * config['train_dataset']['batch_size'],
        leave=True,
...
def main(config_, save_path, args):
    ...
    config['train_dataset']['dataset']['args']['root_path_1'] = os.path.join(data_path, 'all_train_meta_data/train/img')
    config['train_dataset']['dataset']['args']['root_path_2'] = os.path.join(data_path, 'all_train_meta_data/train/mask')
    eval_task = config['test_task']
    config['val_dataset']['dataset']['args']['root_path_1'] = os.path.join(data_path, f'{eval_task}/eval/img')
    config['val_dataset']['dataset']['args']['root_path_2'] = os.path.join(data_path, f'{eval_task}/eval/mask')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default="/root/SAM_Adapter_MAML/configs/cod-sam-vit-b-OSAM.yaml")
